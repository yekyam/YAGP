...
class Args
{
	 ...
	 * \note Assert is used here because argument parsing is the first thing in any program and defines program
	 * behavior. An error state here should in fact fail early.
	 */
	 ...
	 */
	template <> std::optional<int> get<int>(std::string_view option_name, bool is_verbose) const noexcept
	{
		...
		int int_value{};
		auto [p, res] =
		    std::from_chars(str_view_value.data(), str_view_value.data() + str_view_value.size(), int_value);
		if (res == std::errc{})
			return int_value;
		return std::nullopt;
	}
	 ...
	 */
	template <> std::optional<float> get<float>(std::string_view option_name, bool is_verbose) const noexcept
	{
	 ...
	 */
	template <> std::optional<bool> get<bool>(std::string_view option_name, bool is_verbose) const noexcept
	{
	...
	/*
	 * \fn std::optional<std::string_view> get(std::string_view option_name, bool is_verbose = false) const
	 * noexcept; \brief Attempts to get the int value specified by option_name \param std::string_view option_name
	 * The name of the option \param bool is_verbose The verbosity of the function \return An std::optional
	 * containing the std::string_view value or nullopt
	 */
	template <>
	std::optional<std::string_view> get<std::string_view>(std::string_view option_name,
